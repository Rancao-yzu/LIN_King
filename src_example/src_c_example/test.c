...
int main(int argc, char* argv[])
{
    ...
    ////自定义序列号打开设备
    //if (!VCI_OpenDeviceByName(DevType, "A001", &DevIdx)) {
    //     printf("VCI_OpenDevice failed\n");
    //    }else
    //  printf("A001 open success!  devIndex is: %d  \n" ,DevIdx);
	...
	filter.size = sizeof(ZCAN_FILTER) * 3;
	filter.table[0].type = 0; //std
	filter.table[0].sid = 0x100;
	filter.table[0].eid = 0x100;
	
	filter.table[1].type = 0; //std
	filter.table[1].sid = 0x200;
	filter.table[1].eid = 0x300;
	
	filter.table[2].type = 1;   //ext
	filter.table[2].sid = 0x7ff1;
	filter.table[2].eid = 0x7ff2;
	if (!VCI_SetReference(DevType, DevIdx, 0, CMD_CAN_FILTER, &filter)) // 滤波
	{   
		printf("CMD_CAN_FILTER fail\n");
	}*/
